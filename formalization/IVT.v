...
Section IVT.
  ...
  Proof.
    ...
    pose proof (e (eps / d2) (real_half_gt_zero  eps p1)).
    rewrite p2 in H.
    destruct H.
    ...
    contradict H0.
    pose proof (real_gt_half eps p1).
    auto with real.
  ...
  Proof.
    ...
    assert (W_is_non_empty (fun z => a<=z<=b /\ f z < c)) as nepty.
    exists a; constructor 1; auto with real.
    destruct p as [t1 t2]; exact t1.
    assert (W_is_bounded_above (fun z => a<=z<=b /\ f z < c)) as upbd.
    exists b.
    ...
    assert (t1 : eps / d2 > real_0) by auto with real.
    ...
    apply r3 in l1.
    rewrite l1.
    replace u with (real_0+u) at 1 by ring;
      replace (u+δ/d2) with (δ/d2+u) by ring.
    rewrite <- (real_metric_inv real_0 (δ/d2) u).

    destruct (dist_prop real_0 (δ/d2)) as [_[_ u3]];
      rewrite (u3 (real_half_gt_zero  δ o3)).
    ring_simplify; apply @real_gt_half ; exact o3.

    apply r2 in l2.
    rewrite l2.
    replace u with (u+real_0) at 1 by ring.
    rewrite <- (real_metric_plus_inv real_0 (δ/d2) u).
    destruct (dist_prop real_0 (δ/d2)) as [_[_ u3]];
      rewrite (u3 (real_half_gt_zero δ o3)).
    ring_simplify; apply @real_gt_half; exact o3.
    ...
    rewrite H.
    pose proof (real_metric_inv real_0 (b-u) u) as H1.
    replace (real_0+u) with u in H1 by ring.
    replace (b-u+u) with b in H1 by ring.
    rewrite <- H1.
    assert (b-u > real_0) as H2 by auto with real.
    destruct (dist_prop real_0 (b-u)) as [_[_ u3]].
    apply u3 in H2.
    rewrite H2.
    assert ((u+δ/d2) +-u > b+-u).
    ...
    apply l3 in r1.
    rewrite r1.
    pose proof (real_half_gt_zero  δ o3).
    assert (u+δ/d2 > u+real_0).
    apply (real_lt_plus_lt); auto.
    replace (u+real_0) with u in H1 by ring.
    pose proof (real_le_lt_lt a u (u+δ/d2) mem H1); auto with real.
    ...
    assert (u+δ/d2 > u+real_0).
    apply (real_lt_plus_lt); auto.
    replace (u+real_0) with u in H2 by ring.
    pose proof (real_le_lt_lt a u (u+δ/d2) mem H2); auto with real.
    ...
    ring_simplify in H2.
    pose proof (real_gt_half eps o1).
    ...
    replace (-u + u) with real_0 in H3 by ring.
    replace ( - u + (u + δ / d2)) with (  δ / d2) in H3 by ring.
    
    pose proof (real_half_gt_zero  δ o3).
    contradict H4.
    exact (real_lt_nlt (δ/d2) real_0 H3).
    ...
    rewrite <- c1.
    rewrite  (real_metric_inv u (u-δ/d2) (δ/d2)).
    replace (u - δ / d2 + δ / d2) with u by ring.
    rewrite (real_metric_inv (u+δ/d2) u (-u)).
    replace (u + δ / d2 + - u) with (δ/d2) by ring;
      replace (u+-u) with real_0 by ring.
    destruct (dist_prop real_0 (δ/d2)) as [_[_ u3]].
    rewrite dist_symm.
    rewrite (u3 (real_half_gt_zero  δ o3)).
    ring_simplify; apply real_gt_half; exact o3.
    ...
    (* case where Max is a *)
    pose proof (real_half_gt_zero  δ o3).
    apply (real_lt_plus_lt u (real_0) (δ/d2)) in H0.
    ring_simplify in H0.
    ...
    pose proof (real_gt_half δ o3).
    rewrite<- c2.
    exact (real_le_lt_lt (dist u a) (δ/d2) δ H2 H3).
    (**)
    ...
    apply (real_lt_plus_lt (u-δ/d2) real_0 (δ/d2)) in H1.
    ring_simplify in H1.
    destruct mem.
    ...
    apply (real_lt_plus_r_lt c real_0 (eps/d2)) in t1.
    ring_simplify in t1.
    ...
    apply real_lt_plus_lt; exact (real_half_gt_zero  δ o3).
    rewrite <- l2; exact mem2.

    (*** prove that x is an upperbound *)
    assert (W_is_upper_bound (fun z : Real => a<=z<=b /\ f z < c) x).
    unfold W_is_upper_bound.
    intros z [q1 q2].
    ...
    apply (real_lt_plus_lt c real_0 (eps/d2)) in t1.
    ring_simplify in t1.
  ...
  Proof.
    intros.
    unfold opp_fun.
    pose proof (real_metric_inv (- f x) (- f y) (f x + f y)) as A.
  ...
  Definition uniq (f : Real -> Real) (a b : Real)
    := a<b /\ f a < real_0 < f b /\ exists! z, (a<z<b) /\ f z = real_0.

  Lemma uniq_pick : forall (f : Real -> Real) (a b c d : Real), continuous f ->
                                                                uniq f a d ->
                                                                b < c -> a < b -> c < d  -> (f b < real_0) \/ (real_0 < f c).
  Proof.
    intros.
    destruct H0 as [o0 [[o1 o2] o3]].
    destruct (real_total_order (f b) real_0) as [p|[p|p]];
      destruct (real_total_order (f c) real_0) as [q|[q|q]].
    + (* f(b) < 0*)
      left; exact p.
    + (* f(b) < 0 *)
      left; exact p.
    ...
    + (* f(b) = 0 and f(c) < 0: show that is a root in [c,d] 
     and that the root r is equal to b *)
      pose proof (W_IVT_l f H c d H3 real_0 (conj q o2)) as [r [c1 c2]].
    ...
    + (* f(b) > 0 and f(c) < 0 *)
      pose proof (W_IVT_l f H a b H2 real_0 (conj o1 p)) as X.
      pose proof (W_IVT_l f H c d H3 real_0 (conj q o2)) as Y.
  ...
  Definition real_3 := IZreal 3.
  Definition dreal_3 : real_3 <> real_0.
  Proof.
    assert (3>0)%Z.
    intuition.
    exact (real_gt_neq real_3 real_0 (IZreal_pos 3 H)).
  Defined.

  Definition talve (a b c d : Real)
    := a <= b /\ b < c /\ c <= d /\ dist b c =(real_2 * dist a d) / dreal_3.
  ...
  Lemma real_le_mult_r_pos_le : forall c a b, c > real_0 -> a <= b -> a*c <= b*c.
  Proof.
    intros c a b p q.
    destruct q as [q|q].
  ...
  Proof.
    ...
    pose proof
         (real_lt_le_lt (dist c d) (dist b e * prec 1) (dist a f * prec n * prec 1) q p).
  ...
  Proof.
    ...
    pose proof (real_le_lt_lt a b e p1 p2).
    ...
    replace (real_2 * (real_2 * dist a f / dreal_3) / dreal_3) with
        (real_2 * (real_2 * dist a f */ dreal_3)*/dreal_3) by auto.
    
    replace (real_2 * (real_2 * dist a f */ dreal_3)*/dreal_3) with
        (dist a f * (real_2 * real_2  */ dreal_3 */dreal_3)) by ring.

    apply (real_lt_mult_pos_lt).

    + rewrite (lt_metric a f QQ).
      apply (real_lt_plus_lt (-a) a f) in QQ.
      replace (-a + a) with real_0 in QQ by ring.
    ...
    + assert (3 > 0)%Z as Q1 by intuition.
      ...
      replace real_3 with (IZreal 3) by auto.
      replace real_2 with (IZreal 2) by auto.
      unfold IZreal.
      unfold IPreal.
      unfold IPreal_2.
      apply (real_lt_add_r ( - (real_1 + real_1) * (real_1 + real_1) * (real_1 + real_1))).
      replace ((real_1 + real_1) * (real_1 + real_1) * (real_1 + real_1) +
               - (real_1 + real_1) * (real_1 + real_1) * (real_1 + real_1)) with real_0 by ring.
      replace ((real_1 + (real_1 + real_1)) * (real_1 + (real_1 + real_1)) +
  - (real_1 + real_1) * (real_1 + real_1) * (real_1 + real_1)) with real_1 by ring.
      exact real_1_gt_0.
  Qed.

  Lemma inv_unif : forall (a b : Real) (c : a <> real_0) (d : b <> real_0), a = b -> real_inv c = real_inv d.
  Proof.
    intros.
    induction H.
  ...
  Proof.
    ...
    + intros [c|c].
      ++ exists ((real_2*a+b)/dreal_3); exists b.
         ...
         +++
           ...
           (* replace (real_2+real_1%Real) with (real_3%Real) in H3. *)

           (* path on neq *)
           assert (real_2 + real_1 = real_3) as path by (unfold real_2, real_3; unfold IZreal;  unfold IPreal; unfold IPreal_2; ring).
           assert (real_2 + real_1 <> real_0) as Path.
           rewrite path.
           apply real_gt_neq.
           auto with real.
           rewrite <- (irrl _ Path (real_gt_neq (real_2 + real_1) real_0
                                               (eq_ind (real_0 + real_0) (fun t : Real => t < real_2 + real_1) (real_lt_lt_plus_lt real_0 real_2 real_0 real_1 (H1) H2) real_0
                                                       (real_plus_unit real_0)))) in H3.
           assert (/dreal_3 = /Path).
           apply inv_unif.
         ...
         +++
           ...
           ++++
             ...
             pose proof (W_IVT_l f cont ((real_2*a+b)/dreal_3) b H1 real_0 (conj c H3)).
      ...
      ++ exists a.
         ...
         +++
           ...
           assert (real_1 + real_2 <> real_0) as Path.
           rewrite path.
           apply real_gt_neq.
           auto with real.
           rewrite <- (irrl _ Path (real_gt_neq (real_1 + real_2) real_0
                                               (eq_ind (real_0 + real_0) (fun t : Real => t < real_1 + real_2) (real_lt_lt_plus_lt real_0 real_1 real_0 real_2 H2 (H1 )) real_0
                                                       (real_plus_unit real_0)))) in H3.
           assert (/dreal_3 = /Path).
           apply inv_unif.
           (unfold real_2, real_3; unfold IZreal; unfold IPreal; unfold IPreal_2; ring).
         ...
         +++
           ...
           ++++
             ...
             pose proof (W_IVT_l f cont a ((a+real_2*b)/dreal_3) H0 real_0 (conj H2 c)).
             destruct H4 as [x [[Q0 Q1] Q2]].
    ...
    +
      ...
      ++
        ...
        replace (real_1+real_1) with (real_2) in t1 by auto.
        assert (3>0)%Z by intuition.
        apply (IZreal_pos 3) in H0.
        assert (/dreal_3 > real_0) by apply  real_pos_inv_pos2.
  ...
  Proof.
    ...
    apply (M_lift_dom ({a' : Real & {b' : Real | uniq f a' b' /\ talve a a' b' b}})); auto.
    intro Q.
    destruct Q as [x [y [P1 P2]]].
    
    pose proof (trisect f x y H P1) as X.
    apply (M_lift ({a' : Real  & ({ b' | uniq f a' b' /\ talve x a' b' y }) } )); auto.
    intros [x' [y' [P1' P2']]].
    exists x'.
    exists y'.
    split; auto.
  ...
  Proof.
    ...
    + apply (M_lift_dom {a : Real & {b : Real | uniq f a b /\ refining real_0 a b real_1 n} }); auto.
      intro X.
      ...
      exists x'; exists y'.
      split; auto.
      exact (refining_S real_0 x x' y' y real_1 n q q').
  Defined. 


  Lemma CIVT : forall (f : Real -> Real),
      continuous f -> uniq f real_0 real_1 -> {z | real_0<z<real_1 /\ f z = real_0}.
  Proof.
    intros f cont u.
    ...
    + (* construct limit *)
      ...
      apply (M_lift ({(a, b)| uniq f a b /\ refining real_0 a b real_1 (S n)})); auto.
      intros [x [y [p q]]].
      exists x.
      unfold uniq in p.
      ...
      ++ split.
         +++ unfold refining in q.
             ...
             pose proof (real_le_lt_lt real_0 x z Q1 H1) as P.
      ...
      ++ unfold refining in q.
         ...
         +++ pose proof (lt_metric x y Q2) as r1.
             ...
             left; exact p2.
         +++ pose proof (real_le_le_le (dist x z) (dist x y) (prec (S n)) F Q4) as H.
             left; exact (real_le_lt_lt (dist x z) (prec (S n)) (prec n) H (prec_S n)).
