...
Section M_Defs.
  ...
  Definition select : forall x y : K, lazy_bool_up x \/ lazy_bool_up y -> M ({ (lazy_bool_up x) } + { (lazy_bool_up y) }).
  Proof.
  ...
  Lemma M_existence_to_classical : forall A (P : A -> Prop), M{x | P x} -> exists x, P x.
  Proof.
    intros.
    apply M_hprop_elim_f.
    intros p q; apply irrl.
