...
Section PartialNablaMonad.
  Definition PNabla : Type -> Type := fun A => {P : A -> Prop | forall a b, P a -> P b -> a = b}. 
  Definition PNabla_fun_map : forall A B (f : A -> B), PNabla A -> PNabla B.
  Proof.
    intros.
    destruct X.
    exists (fun b => exists a : A, x a /\ b = f a).
    intros y z [a [p1 p2]] [b [q1 q2]].
    rewrite p2, q2.
    rewrite (e _ _ p1 q1).
    apply eq_refl.
  Defined.
  ...
  Definition PNabla_functorial_comp : forall A B C (f : A -> B) (g : B -> C),
      PNabla_fun_map _ _ (fun x => g (f x)) = fun x => (PNabla_fun_map _ _ g) ((PNabla_fun_map _ _ f) x).
  Proof.
    intros.
    apply fun_ext.
    intro.
  ...
  Definition PNabla_functorial_id :  forall A, (fun x : PNabla A => x) = PNabla_fun_map A A (fun x => x).
  Proof.
    intros.
    apply fun_ext.
    intro.
  ...
  Definition PNabla_unit : forall A : Type, A -> PNabla A.
  Proof.
    intros.
    exists (fun a => a = X).
    intros; auto.
  ...
  Definition PNabla_mult : forall A : Type, PNabla (PNabla A) -> PNabla A.
  Proof.
    intros.
    destruct X.
    
    exists (fun a => x (PNabla_unit A a)).
    intros.
    destruct e.
    destruct x0.
    destruct e.
