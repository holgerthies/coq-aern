...
Section RealLimit1.
  ...
  Lemma fast_lower_set_nemp : forall f, is_fast_cauchy_p f -> W_is_non_empty _ (fast_lower_set f).
  Proof.
    intros.
    exists (f(0) - prec O).
  ...
  Lemma lower_set_upbd : forall f, is_fast_cauchy_p f -> W_is_bounded_above _ (fast_lower_set f).
  Proof.
    intros.
    exists (f(0) + prec 0).
    intro.
  ...
  Proof.
    intros.
    pose proof (W_complete _ (fast_lower_set f) (fast_lower_set_nemp f H) (lower_set_upbd f H)).
    destruct H0.
    exists x.
