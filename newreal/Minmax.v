...
Lemma Tmax : forall x y, {z | W_M x y z}.
Proof.
  intros.
  apply mslimit.
  + (* max is single valued predicate *)
    ...
    destruct (W_M_Or x y m H) as [[H11 H12]|[H11 H12]];
      destruct (W_M_Or x y m' H') as [[H11' H12']|[H11' H12']];
      try rewrite <- H11; try rewrite <- H11'; auto with Tiny.   
  + (* construct limit *)
    intros.
    ...
    ++ intros [c1|c2].
       +++ (* when x>y-2ⁿ *)
         ...
         ++++ rewrite <- H1.
              ...
              apply (Tlt_plus_lt y T0 (prec n)) in P; ring_simplify in P.
              apply (Tge_le) in H2.
              apply (Tle_lt_lt x y (y+prec n) H2) in P.
              assert (y-prec n < x < y+prec n) by auto.
              pose proof (prec_pos n) as Q.
              rewrite (dist_symm).
              apply (Tmetric_gtgt_sand y x (prec n) Q H0).
       ...
       +++ (* when x<y-2ⁿ *)
         ...
         ++++ rewrite <- H1.
              pose proof (prec_pos n) as P.
              apply (Tlt_plus_lt x T0 (prec n)) in P; ring_simplify in P.
              apply (Tge_le) in H2.
              apply (Tle_lt_lt y x (x+prec n) H2) in P.
              assert (x-prec n < y < x+prec n) by auto.
              pose proof (prec_pos n) as Q.
              rewrite (dist_symm).
              apply (Tmetric_gtgt_sand x y (prec n) Q H0).
           ++++ rewrite H1.
                destruct (dist_zero x0 x0) as [o t]; rewrite (t eq_refl).
                apply prec_pos.
...
Lemma Tmin : forall x y, {z | W_m x y z}.
Proof.
  intros x y.
  pose proof (Tmax (-x) (-y)) as [m rel].
  exists (-m).
  destruct rel as [a [b c]].
  split.
  intro.

  + apply (Tlt_plus_lt (-x-y)) in H.
    ring_simplify in H.
    apply c in H.
    rewrite H; ring.
