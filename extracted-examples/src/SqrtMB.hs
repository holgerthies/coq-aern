...
sqrt_approx :: (HasCurrentPrecision p) => (WithCurrentPrec (CN MPBall) p) -> Prelude.Integer ->
...
sqrt_approx_fast :: (HasCurrentPrecision p) => (WithCurrentPrec (CN MPBall) p) -> Prelude.Integer ->
                    (WithCurrentPrec (CN MPBall) p)
...
restr_sqrt :: (HasCurrentPrecision p) => (WithCurrentPrec (CN MPBall) p) ->
              (WithCurrentPrec (CN MPBall) p)
restr_sqrt x =
  ssr_have __ (\_ ->
    limit (\n ->
