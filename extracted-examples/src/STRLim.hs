...
npow2 n =
  (\fO fS n -> if n Prelude.== 0 then fO () else fS (n Prelude.- 1))
    (\_ -> Prelude.succ 0)
    (\n0 ->
    (Prelude.*) (npow2 n0) (Prelude.succ (Prelude.succ
      0)))
    n
...
is_covert_translation d a h n =
  let {s = h n} in
  map (\b -> (,) (euclidean_plus d (fst b) a) (snd b))
    s

scale_list :: Prelude.Integer -> (([]) Ball) ->
              AERN2.CReal -> ([])
              ((,) Euclidean AERN2.CReal)
scale_list d l l0 =
  case l of {
...
is_covert_scale_down :: Prelude.Integer ->
                        Prelude.Integer -> Is_covert ->
                        Is_covert
...
tn_row n k l =
  (\fO fS n -> if n Prelude.== 0 then fO () else fS (n Prelude.- 1))
    ...
    (\k' ->
    tn_row n k'
      (tn_col n k (sub (sub (npow2 n) k) n1) l))
    k
...
sierpinski_approx_is_covert :: Prelude.Integer ->
                               Is_covert
sierpinski_approx_is_covert n =
  nat_rect t_is_covert (\_ iHn ->
    is_covert_union n2
      (is_covert_union n2
        (is_covert_scale_down n2 n1 iHn)
        (is_covert_translation n2
          (make_euclidean2 (prec n1) real_0)
          (is_covert_scale_down n2 n1 iHn)))
      (is_covert_translation n2
        (make_euclidean2 real_0 (prec n1))
        (is_covert_scale_down n2 n1 iHn))) n
