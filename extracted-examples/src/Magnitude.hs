...
iPReal_2 :: Prelude.Integer -> CReal
iPReal_2 p =
  (\fI fO fH n -> if n Prelude.== 1 then fH () else
                   if Prelude.odd n
                   then fI (n `Prelude.div` 2)
...
magnitude2 :: CReal -> M Prelude.Integer
magnitude2 x =
  let {
   y = (/) x
    ...
    ssr_have __ (\_ ->
      ssr_suff
        ...
        (ssr_have (magnitude1 y)
          (id (\_top_assumption_ -> opp (of_nat _top_assumption_))))))

magnitude :: CReal -> M Prelude.Integer
magnitude x =
